...
class ExcelProcessorApp:
    ...
    def _process_excel(self, file_path):  
        ...
        try:  
            ...
            new_file_name = f"合并后的产品统计表{month}.{day}.xlsx"  # 按需求生成文件名  
    ...
    def _process_main_sheet(self, wb):  
        ...
        for row in range(2, main_ws.max_row + 1):  
            product_name = main_ws[f'C{row}'].value  # 获取商品名
            if product_name:
                main_ws[f'C{row}'].value = normalize_product_name(product_name)

        # 合并相同商品名的销量
        product_sales = {}  # 字典存储商品名和销量
        for row in range(2, main_ws.max_row + 1):  
            product_name = main_ws[f'C{row}'].value
            sales = main_ws[f'F{row}'].value or 0  # 销量
        ...
        # 清空原来的数据并写入合并后的数据
        main_ws.delete_cols(4, 5)  # 删除D和E列
        main_ws.delete_cols(6, 7)  # 删除F列和G列
        
        # 写入合并后的数据
        main_ws.insert_cols(4)  # 在D列位置插入新列
        main_ws.insert_cols(5)  # 在E列位置插入新列
        
        row_idx = 2
        for product_name, total_sales in product_sales.items():
            main_ws[f'C{row_idx}'] = product_name  # 写入商品名
            main_ws[f'F{row_idx}'] = total_sales  # 写入合并后的销量
            row_idx += 1
